...
export default function SignIn(){
  ...
  return <div className={s.centered}><div className={s.auth}>
    ...
    <section className={s.authForm}><div><h2 className={s.display}>Welcome in.</h2><p style={{marginTop:10}}>Connect your school day in under two minutes.</p></div><label className={s.field}>First name<input className={s.input} value={name} onChange={e=>setName(e.target.value)} /></label><Button className={s.google} onClick={go}>Continue</Button><p className={s.fine}>Classroom data syncs from the local backend when Google authorization is available.</p></section>
  </div></div>
}
